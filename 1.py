...
class RobotARM_IMR165_GUI:
    ...
    def create_control_panel(self, parent):
        ...
        # Аварийная кнопка
        ttk.Button(frame, text="АВАРИЙНАЯ ОСТАНОВКА", style='Emergency.TButton',
                   command=lambda: self.emergency_stop("Ручная активация")).pack(fill=tk.X, pady=10)
    ...
    def update_system_state(self):
        ...
        if self.system_state != "emergency":
            getattr(self, "emergency_btn").config(state=state)
